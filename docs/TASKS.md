...
- [x] Reordered `src/index.njk`'s includes to the final target order: Hero → Key Metrics → Selected Product Work → Independent Build (Iqro Land) → About → Operating Principles → Toolbox → Learning in Public → Experience → Education → Contact. Only actual code change was moving the `learning-in-public.njk` include from between Iqro Land/About to between Toolbox/Experience.
- [x] Verified `site.json`'s `navLinks`/`footerNavLinks` (`#about`, `#case-studies`, `#principles`, `#experience`, `#contact`) all still exist and remain in ascending document order after the reorder — no data change needed. Mobile nav panel is data-driven off the same arrays, so it required no separate check.
